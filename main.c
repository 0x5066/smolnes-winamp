  ...
  default: // $8000...$ffff ROM
    ...
    if (write)
      ...
      case 1: // mapper 1
        ...
        {
          // Note to future dumbass: the ? operator is ternary
        ...
        : tmp == 6   ? &chrbank1
          ...
          // Update PRG banks.
          tmp = mmc1_ctrl / 4 % 4 - 2;
          prg[0] = !tmp ? 0 : tmp == 1 ? prgbank
                                       : prgbank & ~1;
          prg[1] = !tmp ? prgbank : tmp == 1 ? rombuf[4] - 1
                                             : prgbank | 1;
...
loop:
  ...
  opcode = read_pc(); // Read PC and stuff it into the current opcode
  uint8_t opcodelo5 = opcode & 31; // Por que? 
  // This is some bullshit that involves bitwise operations
  // Not exactly sure what opcodelo5 is at the moment, I'll get back to you on that

  // On adding "illegal" opcodes:
  // I believe the empty cases are where they would go...running AccuracyCoin crashes on illegal opcode tests

  switch (opcodelo5)
  {
  case 0:
    if (opcode & 0x80)
    // If the opcode is ANDed with 0x80
    // I'm beginning to think this is for "efficiency" purposes and keeping it under 5KB for that sick ass art.
    // It's obtuse as hell BUT I guess it works for our purposes.
    { // LDY/CPY/CPX imm
      read_pc();
      nomem = 1;
      goto nomemop;
    ...
    switch (opcode >> 5) // JMP, JSR, RTI, BRK, RTS
    /* 
    To better understand this:
    I have no idea why it's written like this BUT it works.
    If you have an RTI ($40) opcode in this section, it is shifted right >> 5 times which gives you the case number. --> ergo, case 2
    ...
    */
    ...
    cycles += 4;
    break; // please don't actually *break*

  case 16: // BPL, BMI, BVC, BVS, BCC, BCS, BNE, BEQ
    read_pc();
    if (!(P & mask[opcode >> 6]) ^ opcode / 32 & 1) // I'm not going to even try to understand this.
    {
  ...
  case 26: // opcodelo5 ??? --> 26
    ...
    case 13: //NOP impl (get used to seeing these) [$DA >> 4 = 13]
      break;

    case 14: // NOP [$EA >> 4 = 12]
    // EA bytes, it's in the game
      break;

    case 15: //NOP impl [$FA >> 4 = 15]
  ...
  case 2: // Potentially "JAM", also known as "KIL"
    // A2 is a perfectly "normal" LDX # instruction so there needs to be a check for that
    // 82 C2 and E2 are illegal NOPs and probably should be handled differently.
  ...
  case 28:
  case 29:
  case 30: // Absolute, X-indexed    +2/3
    addr_lo = read_pc();
    addr_hi = read_pc();
    if (opcodelo5 < 25)
      goto opcode;
  ...
  {
    if (ppumask & 24)
    { // If background or sprites are enabled.
      if (scany < 240)
      {
        if (dot - 256 > 63u)
        { // dot [0..255,320..340]
